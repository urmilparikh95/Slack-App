	...
	return new Promise(function (resolve, reject) {
		...
		gitpull(res[res.length - 1], function (err, consoleOutput) {
			if (err) {
				...
				var list = [];
				walkSync(res[res.length - 1],[], list);
				if (list && list.length > 0){
					resolve(list);
				}else{
					reject("Oops! Could not fetch files from repository!")
				}
				});
			} else {
...
function walkSync(dir, filelist, list) {
	...
	if (fs.existsSync(dir)){
		...
		files.forEach(function(file) {
			...
			filelist = walkSync(dir + '/' + file, filelist, list);
			}
			else {
