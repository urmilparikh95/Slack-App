...
register = function(response, convo) {
	convo.say("Let's configure your Github Repository..")
	convo.ask("Enter the link of github repository which you want to monitor. \n Eg. `https://github.ncsu.edu/dummy_user/dummy_repo1/`", function(response, convo) {
		repo_link = response.text;
		if (repo_link && repo_link.length>0 && repo_link[0] == "<" && repo_link[repo_link.length - 1] == ">"){
			var extracted_repo_link = repo_link.substring(1, repo_link.length-1);
			...
			saveUserGitInfo(response.user, extracted_repo_link, convo);
		}else{
...
saveUserGitInfo = function(user, repo_link, convo) { 		
	...
	console.log("Repo Link: "+repo_link);
		...
		database.getUserInfo(message.user).then(function (result)
		{
			...
			github.getFilesFromRepo(result.repo_link).then(function (repo)
			{
				console.log("Repo: "+JSON.stringify(repo));
					droid_service.getUiLibraries(repo).then(function (recommendations) 
					{
		...
		console.log("Repo: "+JSON.stringify(repo));
			droid_service.getUiLibraries(repo).then(function (recommendations) 
			{
				if (recommendations.status != undefined && recommendations.status === "error"){
					...
					for (let key of Object.keys(recommendations)) {
						if (droid_data.hasOwnProperty(key) && recommendations[key].length > 0){
							convo.say(droid_data[key]);	
						}
					}
				}
			}).catch(function(error){
				convo.say("Darn! Recommendations could not be fetched!");
				convo.say("Please check if the following repository exists and contains Android Code: `"+repo_link+"`");
				console.log("Error: "+error);
			});

	}).catch(function(error){
		convo.say("Darn! Please check if the following repository exists and contains Android Code: `"+repo_link+"`");
		convo.say("Use `Configure` to configure a new repostitory!")
		console.log("Error: "+error);
	});
}
...
function suggesCodeLibraries(repo_link, credential, convo){
	...
	{
		console.log("Repo: "+JSON.stringify(repo));
			droid_service.getCodeLibraries(repo).then(function (recommendations) 
			{
				if (recommendations.status != undefined && recommendations.status === "error"){
					convo.say(recommendations.message);
				}else{
...
function suggesCodeRefactorings(repo_link, credential, convo){
	...
	{
		console.log("Repo: "+JSON.stringify(repo));
			droid_service.getCodeRefactoringSuggestions(repo).then(function (recommendations) 
			{
				if (recommendations.status != undefined && recommendations.status === "error"){
					convo.say(recommendations.message);
				}else{
...
function suggestAllImprovements(repo_link, credential, convo){
	...
	{
		// console.log("Repo: "+JSON.stringify(repo));
			droid_service.getAllSuggestions(repo).then(function (recommendations) 
			{
				if (recommendations.status != undefined && recommendations.status === "error"){
					convo.say(recommendations.message);
				}else{
